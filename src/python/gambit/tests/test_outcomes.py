...
class TestGambitOutcomes(object):
    ...
    def test_game_outcomes_index_by_outcome(self):
        "Test find an outcome by providing an int"
        self.game.outcomes[0].label = "trial"
        s1 = self.game.outcomes[self.game.outcomes[0]]
        assert s1.label == "trial"
    ...
    def test_game_outcome_index_exception_outcome(self):
        "Test to verify when an outcome object is not from the same game"
        self.game_2 = gambit.new_table([2,2])
        assert_raises(IndexError, self.game.outcomes.__getitem__, self.game_2.outcomes[0])
